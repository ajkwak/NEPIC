 ...
 * @author AJ Parmidge
 */
public class OrderedDataSet implements DataSet {
    private String name = ""; // Default is empty string.
    private final TreeSet<Point> data;
     ...
     */
    public OrderedDataSet() {
        this.data = new TreeSet<Point>(new DataPointComparator());
    }
    ...
    public Iterator<Point> iterator() {
        return new Iterator<Point>() {
            ...
            @Override
            public Point next() {
                Point nextPt = dataItr.next();
                return new Point(nextPt.x, nextPt.y);
            }
    ...
    public double interpolateY(int x) {
        ...
        Point floorPt = data.floor(new Point(x, 0));
        if (floorPt.x == x) {
            return floorPt.y;
        } else {
            Point ceilingPt = data.ceiling(new Point(x, 0));
            return new Line(floorPt, ceilingPt).getY(x);
        }
    }
    ...
    private class DataPointComparator implements Comparator<Point> {

        @Override
        public int compare(Point p1, Point p2) {
            int diffX = p1.x - p2.x;
            if (diffX == 0) { // TODO: move this
                throw new IllegalArgumentException(
                        "Cannot have two data points with the x value " + p1.x);
            }
            return diffX;
        }
    }
