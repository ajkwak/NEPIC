...
/**
 * Decodes single and multi-image TIFF files. The LZW decompression code was contributed by Curtis
 * Rueden.
 */
public class TiffDecoder {// from ImageJ ij.io.TiffDecoder class
    ...
    /**
     * Save the image description in the specified FileInfo. ImageJ saves spatial and density
     * calibration data in this string. For stacks, it also saves the number of images to avoid
     * having to decode an IFD for each image.
     */
    ...
    /**
     * Returns a double containg the value represented by the specified <code>String</code>. Taken
     * from ImageJ ij.util.Tools class (need for TiffDecoder)
     * 
     * @param s the string to be parsed.
     * @param defaultValue the value returned if <code>s</code> does not contain a parsable double
     * @return The double value represented by the string argument or <code>defaultValue</code> if
     *         the string does not contain a parsable double
     */
    public static double parseDouble(String s, double defaultValue) {// from ImageJ ij.util.Tools
                                                                     // class (need for TiffDecoder)
        if (s == null)
            return defaultValue;
        try {
    ...
    void decodeNIHImageHeader(int offset, FileInfo fi) throws IOException {
        ...
        if (nImages >= 2 && (fi.fileType == FileInfo.GRAY8 || fi.fileType == FileInfo.COLOR8)) {
            ...
            in.readShort(); // CurrentSlice (skip)
        ...
        for (int i = 0; i < nEntries; i++) {
            ...
            case SAMPLES_PER_PIXEL:
                ...
                if (value == 3 && fi.fileType != FileInfo.RGB48)
                    fi.fileType = fi.fileType == FileInfo.GRAY16_UNSIGNED ? FileInfo.RGB48
                            : FileInfo.RGB;
