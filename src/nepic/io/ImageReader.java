...
//TODO: put back all methods that I can so can read as many images as possible.  Figure out where need to put code so can log unusual events

/**
 * Reads raw 8-bit, 16-bit or 32-bit (float or RGB) images from a stream or URL.
 */
public class ImageReader {// from ImageJ ij.io.ImageReader clss
    // TODO: would need to get BitBuffer from ImageJ to be able to read in compressed TIFF files
    ...
    float[] read32bitImage(InputStream in) throws IOException {
        ...
        while (totalRead < byteCount) {
            ...
            if (fi.intelByteOrder)
                ...
                for (int i = base; i < pmax; i++) {
                    tmp = (((buffer[j] & 0xff) << 24) | ((buffer[j + 1] & 0xff) << 16)
        ...
        while (totalRead < byteCount) {
            ...
            for (int i = base; i < (base + pixelsRead); i++) {
                ...
                if (fi.intelByteOrder)
                    ...
                    tmp = ((b8 << 56) | (b7 << 48) | (b6 << 40) | (b5 << 32) | (b4 << 24)
                            | (b3 << 16) | (b2 << 8) | b1);
    ...
    int[] readCompressedChunkyRGB(InputStream in) throws IOException {
        ...
        for (int i = 0; i < fi.stripOffsets.length; i++) {
            if (i > 0) {
                long skip = (fi.stripOffsets[i] & 0xffffffffL)
     ...
     * short, int or float). Returns null if there was an IO exception. Does not close the
     * InputStream.
     */
     ...
     * Specification: http://partners.adobe.com/asn/developer/pdfs/tn/TIFF6.pdf (page 61)
     * 
    ...
    /*
     * void debug(String label, InputStream in) { int offset = -1; if (in instanceof
     * RandomAccessStream) { try { offset = ((RandomAccessStream)in).getFilePointer(); }
     * catch(Exception e) {} } IJ.log(label+": debug: offset="+offset+", fi="+fi); }
     */
}
